...
v7.1.8 起，交付包内不再携带 PDF/MD 说明文档或模型配置 `.env` 文件；本目录只保留本地启动所需文件。
...
```bash
docker load -i codesetarena-student-v7.1.8.image.tar
docker compose up
```
...
首次打开后，请在“设置”页手动填写并保存 Base URL、API Key 和模型列表。
