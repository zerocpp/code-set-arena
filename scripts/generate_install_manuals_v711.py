"""Generate Chinese Docker installation and operation manuals for v7.1.1."""
...
def platform_install_section() -> list:
    return [
        h1("一、安装 Docker"),
        p("CodeSetArena v7.1.1 以 Docker 镜像形式交付。学生端和助教端是两个独立容器，数据目录也相互独立。"),
        flow_diagram(["解压离线包", "安装 Docker", "载入镜像", "启动容器", "打开网页"], BLUE),
        Spacer(1, 0.22 * cm),
        info_box(
            "离线包优先",
            "课程可发放 codesetarena-docker-offline-v7.1.1.tar.gz。这个包应包含 Docker Desktop 安装包、Windows WSL MSI、Linux 辅助脚本，以及学生端/助教端本地镜像交付包。Windows WSL 2 通常使用 linux-amd64；Apple Silicon 或 ARM64 Linux 可使用通用包中的 linux-arm64。网络较差时，先用这些本地文件安装，不要让每台电脑现场重新下载。",
            BLUE,
        ),
        code_block(
            """
# 解压并校验离线包
tar -xzf codesetarena-docker-offline-v7.1.1.tar.gz
cd docker-offline-v7.1.1
shasum -a 256 -c SHA256SUMS.txt
        ...
        info_box(
            "中国大陆网络提示",
            "助教可在网络较好的机器上运行 scripts/prepare_docker_offline_bundle_v711.py 生成完整离线包。确需本地构建时，可设置 PIP_INDEX_URL 为学校或可信镜像源；Docker Hub 镜像加速请按学校或单位提供的 registry mirror 配置，不建议随意复制未知镜像地址。",
            AMBER,
        ),
        code_block(
            """
...
docker load -i codesetarena-student-v7.1.1.image.tar
docker load -i codesetarena-teacher-v7.1.1.image.tar
            """
        ),
    ]
...
def student_manual() -> list:
    story: list = [
        ...
        *platform_install_section(),
        PageBreak(),
        h1("二、启动学生端"),
        p("推荐使用课程离线交付包启动，避免学生电脑直接访问 Docker Hub。如果使用完整 Docker 离线包，学生端交付包位于 docker-offline-v7.1.1/codesetarena/ 目录。Windows WSL 2 和普通 x86_64 Linux 使用 linux-amd64；如果 Docker Server 输出 linux/arm64，请改用通用包中的 linux-arm64 版本。以下命令在包含学生端交付包的目录执行。"),
        code_block(
            f"""
docker version --format '{{{{.Server.Os}}}}/{{{{.Server.Arch}}}}'
...
docker run -d --name codesetarena-student-v711 \\
  -p 8000:8000 \\
  ...
  codesetarena-student:{VERSION}
        ...
        key_value_table(
            [
                ("停止", "docker compose down，或 docker stop codesetarena-student-v711。"),
...
def teacher_manual() -> list:
    story: list = [
        ...
        *platform_install_section(),
        PageBreak(),
        h1("二、启动助教端"),
        p("推荐使用课程离线交付包启动。若使用完整 Docker 离线包，助教端交付包位于 docker-offline-v7.1.1/codesetarena/ 目录。Windows WSL 2 和普通 x86_64 Linux 使用 linux-amd64；如果 Docker Server 输出 linux/arm64，请改用通用包中的 linux-arm64 版本。助教端默认使用 8010 端口，避免和学生端 8000 冲突。"),
        code_block(
            f"""
docker version --format '{{.Server.Os}}/{{.Server.Arch}}'
...
docker run -d --name codesetarena-teacher-v711 \\
  -p 8010:8010 \\
  ...
  codesetarena-teacher:{VERSION}
        ...
        key_value_table(
            [
                ("停止", "docker compose down，或 docker stop codesetarena-teacher-v711。"),
