"""Prepare offline Docker installers and CodeSetArena local packages for v7.1.4."""
...
codesetarena/
  ...
  codesetarena-student-local-{VERSION}-linux-arm64.tar.gz
  codesetarena-teacher-local-{VERSION}-linux-arm64.tar.gz
manifest.json
SHA256SUMS.txt
```
...
学生端和助教端本地交付包在 `codesetarena/` 目录。Windows WSL 2 和普通 x86_64 Linux 使用 `linux-amd64`；Apple Silicon 或 ARM64 Linux 使用 `linux-arm64`。先确认 Docker Server 架构：
